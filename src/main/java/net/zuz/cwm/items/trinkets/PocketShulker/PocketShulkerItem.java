...
public class PocketShulkerItem extends TrinketItem implements TrinketRenderer {
    ...
    public void tick(ItemStack stack, SlotReference slot, LivingEntity entity) {
        ...
        if(entity.getY() <= -10)
        {
            entity.addStatusEffect(new StatusEffectInstance(StatusEffects.LEVITATION,800,0, true, false));
        }
        if(entity.getY() >= 255)
        {
            entity.addStatusEffect(new StatusEffectInstance(StatusEffects.SLOW_FALLING,1600,0, true, false));
        }
    }
    ...
    {
        Registry.register(Registry.ITEM, new Identifier("cwm", "shulk"), new TntFrogItem(new Item.Settings().group(group).rarity(Rarity.RARE).maxCount(1).fireproof()));
    }
}
